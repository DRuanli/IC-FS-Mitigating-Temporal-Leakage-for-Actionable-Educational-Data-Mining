...
def run_one_seed(df_raw, seed, horizon):
    ...
    ius_full_dep = f1_full_dep * ar_full
    ius_notemp_dep = f1_notemp_dep * ar_notemp
    ...
    return {"seed": seed, "horizon": horizon,
             ...
             "IUS_full_paper": best_ius * 100,
             "IUS_notemp_paper": best_ius_nt * 100,
             "IUS_full_deploy": ius_full_dep * 100,
             "IUS_notemp_deploy": ius_notemp_dep * 100,
             "AR_full": ar_full, "AR_notemp": ar_notemp,
...
def main():
    ...
    for col in ["IUS_full_paper","IUS_notemp_paper"]:
        v = df[col].values
    ...
    for col in ["IUS_full_deploy","IUS_notemp_deploy"]:
        v = df[col].values
        print(f"  {col:<20}: mean={v.mean():.2f}  95%CI=[{np.percentile(v,2.5):.2f},{np.percentile(v,97.5):.2f}]")

    print("\n--- Wilcoxon: IC-FS(full) > IC-FS(-temporal) under DEPLOYMENT ---")
    a = df["IUS_full_deploy"].values
    b = df["IUS_notemp_deploy"].values
